...
let numbers = [1, 2, 3, 4]
let evenNumb = numbers.filter((i) => i % 2 !== 0)
let numDubble = evenNumb.map((i) => i * 2)

console.log(numDubble)
...
const shortMovi = movies
  .filter((movie) => movie.title.length < 6)
  .map((movie) => movie.title)

console.log(shortMovi)

// 2. Create an array of movie titles with long movie titles

const longMovi = movies
  .filter((movie) => movie.title.length > 6)
  .map((movie) => movie.title)
...
const ratingHight = movies
  .filter((rating) => rating.rating > 6)
  .map((rating) => rating.rating)
console.log(ratingHight)
...
const counterContain = movies
  .filter((tit) => tit.title.includes('Surfer' || 'Alien' || 'Benjamin'))
  .map((tit) => tit.title)
movies.filter((movie) => {
  if (movie.title.includes('Surfer')) {
    ...
    numbKeywords.Benjamin++
  }
  return numbKeywords
})

console.log(numbKeywords)
...
const averRating = movies.reduce((sum, ratin) => {
  return sum + ratin.rating
}, 0)
